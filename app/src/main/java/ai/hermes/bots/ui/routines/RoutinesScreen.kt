                        ...
                        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                ...
                                Column(Modifier.weight(1f)) {
                                    ...
                                    Text(
                                        Humanize.cron(job.scheduleText),
...
private fun RoutineDialog(
    ...
    val presets = listOf(
        "Every morning 9 AM" to "0 9 * * *",
        "Every 30 min" to "*/30 * * * *",
        "Weekdays 6 PM" to "0 18 * * 1-5",
    )
    var custom by remember { mutableStateOf(presets.none { it.second == initialSchedule }) }
    AlertDialog(
            ...
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Row(
                    ...
                    presets.forEach { (label, expr) ->
                        FilterChip(
                            selected = !custom && schedule == expr,
                            onClick = { schedule = expr; custom = false },
                            label = { Text(label) },
                        )
                    }
                    FilterChip(
                ...
                if (custom) {
                    OutlinedTextField(
                        ...
                        label = { Text("Custom schedule (cron)") },
                        singleLine = true,
                    )
                } else {
                    Text(
